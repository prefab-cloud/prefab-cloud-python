...
from tests.helpers import EmptyResolver
...
WVAL = Prefab.ConfigValue(weighted_values=Prefab.WeightedValues(weighted_values=[Prefab.WeightedValue(weight=1000, value=VALUE1)]))
...
def test_rollup_counts_properly(mock_time):
    ...
    evaluation_rollup.record_evaluation(
        Evaluation(config=CONFIG1, value=VALUE1, value_index=31, config_row_index=0, resolver=EMPTY_RESOLVER,
                   context=EMPTY_CONTEXT))
    evaluation_rollup.record_evaluation(
        Evaluation(config=CONFIG1, value=VALUE2, value_index=32, config_row_index=0, resolver=EMPTY_RESOLVER,
                   context=EMPTY_CONTEXT))
    evaluation_rollup.record_evaluation(
        Evaluation(config=CONFIG2, value=VALUE1, value_index=31, config_row_index=1, resolver=EMPTY_RESOLVER,
                   context=EMPTY_CONTEXT))
    # do this one twice
    evaluation_rollup.record_evaluation(
        Evaluation(config=CONFIG2, value=VALUE2, value_index=32, config_row_index=1, resolver=EMPTY_RESOLVER,
                   context=EMPTY_CONTEXT))
    evaluation_rollup.record_evaluation(
        Evaluation(config=CONFIG2, value=VALUE2, value_index=32, config_row_index=1, resolver=EMPTY_RESOLVER,
                   context=EMPTY_CONTEXT))
    telemetry = evaluation_rollup.build_telemetry()
    sorted_summaries = sort_summaries(telemetry.summaries)
    ...
    assert sort_counters(first_summary.counters) == sort_counters([Prefab.ConfigEvaluationCounter(count=1,
                                                                                                  config_id=CONFIG1.id,
                                                                                                  config_row_index=0,
                                                                                                  conditional_value_index=31,
                                                                                                  selected_value=VALUE1),
                                                                   Prefab.ConfigEvaluationCounter(count=1,
                                                                                                  config_id=CONFIG1.id,
                                                                                                  config_row_index=0,
    ...
    assert sort_counters(second_summary.counters) == sort_counters([Prefab.ConfigEvaluationCounter(count=1,
                                                                                                   config_id=CONFIG2.id,
                                                                                                   config_row_index=1,
                                                                                                   conditional_value_index=31,
                                                                                                   selected_value=VALUE1),
                                                                    Prefab.ConfigEvaluationCounter(count=2,
                                                                                                   config_id=CONFIG2.id,
                                                                                                   config_row_index=1,
                                                                                                   conditional_value_index=32,
                                                                                                   selected_value=VALUE2)])
...
def test_rollup_works_for_weighted_value():
    ...
    evaluation_rollup.record_evaluation(
        Evaluation(config=CONFIG1, value=WVAL, value_index=31, config_row_index=0,
                   resolver=EMPTY_RESOLVER,
                   context=EMPTY_CONTEXT))
    telemetry = evaluation_rollup.build_telemetry()
    assert len(telemetry.summaries) == 1
    assert telemetry.summaries[0].counters == [Prefab.ConfigEvaluationCounter(count=1,
                                                                              config_id=CONFIG1.id,
                                                                              config_row_index=0,
                                                                              conditional_value_index=31,
                                                                              selected_value=VALUE1,
...
def test_rollup_works_for_confidential():
    evaluation_rollup = EvaluationRollup()
    evaluation_rollup.record_evaluation(
        Evaluation(config=CONFIG1, value=CONFIDENTIAL_VALUE, value_index=31, config_row_index=0,
                   resolver=EMPTY_RESOLVER,
                   context=EMPTY_CONTEXT))
    telemetry = evaluation_rollup.build_telemetry()
    assert len(telemetry.summaries) == 1
    assert telemetry.summaries[0].counters == [Prefab.ConfigEvaluationCounter(count=1,
                                                                              ...
                                                                              config_row_index=0,
                                                                              conditional_value_index=31,
                                                                              selected_value=Prefab.ConfigValue(
                                                                                  string="*****e31d9"))]
...
def sort_counters(counters):
    return sorted(counters, key=lambda obj: (
        obj.config_id, obj.selected_index, obj.config_row_index, obj.conditional_value_index, obj.weighted_value_index,
